...
const COLLATERAL: &[u8] = include_bytes!("../../../fixtures/prod-1/collateral.json");
...
fuzz_target!(|data: &[u8]| {
    let mut cursor = data;
    let Ok(quote) = dcap_verify::SgxQuote::read(&mut cursor) else {
        return;
    };
    let collateral: dcap_verify::SgxCollateral =
        serde_json::from_slice(COLLATERAL).expect("embedded collateral parses");
